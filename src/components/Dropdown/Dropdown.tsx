...
const Dropdown = forwardRef<HTMLDivElement, BaseDropdownProps>(({ children, isVisible, setIsVisible, customClasses, positionAdjustment, toggleRef, customStyling }, ref) => {
    ...
    return (
        ...
        <div ref={dropdownRef} className={`absolute top-full left-0 z-50 text-white bg-color-gray-600 rounded-lg text-sm ${positionClass} ${customClasses || ''}`} style={customStyling ? customStyling : undefined}>
            {children}
        </div>
    );
});
