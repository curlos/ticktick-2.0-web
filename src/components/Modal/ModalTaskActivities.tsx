                    ...
                    {activityList.map((activity) => {
                        ...
                        return (
                            <li key={_id} className="relative m-0 list-none last:mb-[4px] mt-[24px]" style={{ minHeight: '54px' }}>
                                <div className="absolute top-[28px] left-[11px] h-full border-solid border-l-[1px] border-color-gray-100" style={{ height: "calc(100% - 16px)" }}>
                                </div>
                                ...
                                <div className="relative m-0 ml-[40px] break-words" style={{ marginTop: 'unset' }}>
                                    <div className="absolute left-[-40px] w-[24px] h-[24px] bg-primary-10 rounded-full flex items-center justify-center">
                                        {actionType === 'edit' ? (
                                            <Icon name="edit" customClass={"!text-[16px] text-white cursor-pointer bg-yellow-500 rounded-full p-1"} fill={1} />
                                        ) : actionType === 'complete' ? (
                                            <Icon name="done" customClass={"!text-[16px] text-white cursor-pointer bg-emerald-400 rounded-full p-1"} fill={1} />
                                        ) : actionType === 'undone' ? (
                                            <Icon name="redo" customClass={"!text-[16px] text-white cursor-pointer bg-gray-400 rounded-full p-1 -scale-x-100"} fill={1} />
                                        ) : (
                                            <Icon name="add" customClass={"!text-[16px] text-white cursor-pointer bg-blue-500 rounded-full p-1 -scale-x-100"} fill={1} />
                                        )}
                                    </div>
